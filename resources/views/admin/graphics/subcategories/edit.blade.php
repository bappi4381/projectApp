...
@section('title', 'Edit Sub-Category | Graphics Studio')
...
<div class="p-8">
    <div class="mb-10 reveal">
        ...
        <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">Edit Sub-Group</h1>
        <p class="text-slate-400 font-medium">Update the grouping details: {{ $subCategory->name }}</p>
    </div>
    ...
    <div class="glass-card rounded-[32px] border-white/5 shadow-2xl relative overflow-hidden reveal reveal-delay-1 p-10 max-w-2xl">
        <form action="{{ route('admin.graphics.subcategories.update', $subCategory) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $subCategory->name) }}" placeholder="e.g. Clipping Path" required 
