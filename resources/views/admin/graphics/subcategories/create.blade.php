...
@section('title', 'Create Service Group | Graphics Studio')
...
<div class="p-8">
    <div class="mb-10 reveal">
        ...
        <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">Create Service Group</h1>
        <p class="text-slate-400 font-medium text-sm italic">Level 2 in the service hierarchy (e.g., Real Estate Rendering, Product Rendering)</p>
    </div>
    ...
    <div x-data="{ 
        ...
        <form action="{{ route('admin.graphics.subcategories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            @csrf
            ...
            <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10 reveal reveal-delay-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Parent Vertical (Vertical Name in Drawing)</label>
                        <select name="category_id" required class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all select-dark">
                            <option value="">Select Primary Vertical</option>
                    ...
                    <div>
                        <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Group Name (Navigation Sub-Group)</label>
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" value="{{ old('icon', 'ri-stack-line') }}" placeholder="ri-stack-line" 
                            class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all">
                    </div>
                </div>
                ...
                <div class="mt-10 p-6 bg-indigo-500/5 rounded-3xl border border-indigo-500/10 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-[10px] text-slate-500 font-medium">Toggle this to define rich complexity grids, FAQs, and pricing for this group.</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-3 text-xs font-black uppercase tracking-widest" :class="hasDetails ? 'text-emerald-400' : 'text-slate-600'" x-text="hasDetails ? 'Yes' : 'No'"></span>
                    </label>
                </div>
            </div>
            ...
            <div x-show="hasDetails" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-10">
                ...
                <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h2 class="text-xl font-bold text-white tracking-tight">Landing Page Grid Content</h2>
                    </div>

                    <div class="space-y-8">
                        <div>
                            <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Landing Page Description</label>
                            <textarea name="description" rows="4" class="w-full bg-white/5 border border-white/10 rounded-[2rem] px-8 py-6 text-sm font-medium text-slate-300 focus:outline-none focus:border-indigo-500 transition-all resize-none">{{ old('description') }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div x-data="{ preview: null }">
                                <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Before Transformation Image</label>
                            ...
                            <div x-data="{ preview: null }">
                                <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">After Transformation Image</label>
                                <div class="flex items-center gap-6 p-4 bg-white/5 border border-white/10 rounded-2xl">
                                    <div class="w-24 h-24 rounded-xl bg-slate-800 overflow-hidden shrink-0 border border-white/5">
                                        <template x-if="preview"><img :src="preview" class="w-full h-full object-cover"></template>
                                        <template x-if="!preview"><div class="w-full h-full flex items-center justify-center text-slate-600"><i class="ri-image-2-line text-2xl"></i></div></template>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10">
                        <div class="flex items-center justify-between mb-8">
                            <h3 class="text-lg font-bold text-white tracking-tight">Architecture FAQ Builder</h3>
                        ...
                        <div class="space-y-4">
                            <template x-for="(method, index) in methods" :key="index">
                                <div class="p-4 bg-white/5 border border-white/10 rounded-2xl relative group">
                                    ...
                                    <textarea :name="'methods['+index+'][description]'" x-model="method.description" placeholder="Short Description..." class="w-full bg-transparent border-none text-[11px] text-slate-400 focus:ring-0 resize-none h-20"></textarea>
                                </div>
                            </template>
                        </div>
                    </div>
