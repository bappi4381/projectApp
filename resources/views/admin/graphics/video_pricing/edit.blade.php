...
@section('title', 'Edit Video Pricing | Graphics Studio')

@section('content')
<div class="p-8 max-w-7xl mx-auto" x-data="{
    service_name: '{{ $videoPricing->service_name }}',
    pricing_tiers: {{ json_encode($videoPricing->pricing_tiers ?? []) }},
    ...
    <div class="flex items-center gap-4 mb-10">
        ...
        <div>
            <h1 class="text-3xl font-bold text-white tracking-tight">Edit Video Pricing</h1>
            <p class="text-slate-500 text-sm font-medium">Update pricing table for {{ $videoPricing->service_name }}.</p>
        </div>
    </div>

    <form action="{{ route('admin.graphics.video-pricing.update', $videoPricing) }}" method="POST" class="space-y-10">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="flex items-end pb-4">
                    <label class="flex items-center gap-4 cursor-pointer group">
                        <div class="relative">
                            <input type="checkbox" name="is_active" value="1" {{ $videoPricing->is_active ? 'checked' : '' }} class="sr-only peer">
        ...
        <div class="flex justify-end gap-4 pt-6">
            ...
            <button type="submit" class="px-12 py-4 bg-indigo-600 hover:bg-indigo-500 text-white text-[11px] font-black uppercase tracking-widest rounded-2xl transition-all shadow-xl shadow-indigo-500/20 active:scale-95">Update Pricing Table</button>
        </div>
    </form>
</div>
@endsection
