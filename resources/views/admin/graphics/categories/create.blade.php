...
@section('title', 'Create Primary Vertical | Graphics Studio')
...
<div class="p-8">
    <div class="mb-10 reveal">
        ...
        <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">Create Primary Vertical</h1>
        <p class="text-slate-400 font-medium text-sm italic">Level 1 in the service hierarchy (e.g., Image Editing, Video Editing)</p>
    </div>
    ...
    <div x-data="{ 
        ...
        <form action="{{ route('admin.graphics.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            @csrf
            ...
            <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10 reveal reveal-delay-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Icon Class (RemixIcon)</label>
                        <input type="text" name="icon" value="{{ old('icon') }}" placeholder="ri-image-edit-line" 
                            class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all">
                    </div>
                </div>
                ...
                <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10">
                    ...
                    <div class="space-y-8">
                        <div>
                            ...
                            <input type="text" name="hero_heading" value="{{ old('hero_heading') }}" placeholder="e.g. Professional Image Editing"
                        ...
                        <div>
                            ...
                            <textarea name="short_description" rows="2" placeholder="Brief summary of the vertical..." class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-medium text-slate-300 focus:outline-none focus:border-emerald-500 transition-all resize-none mb-6">{{ old('short_description') }}</textarea>
                        </div>

                        <div>
                            <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Video Link (YouTube/Vimeo)</label>
                            <input type="url" name="video_link" value="{{ old('video_link') }}" placeholder="https://..."
                                class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-emerald-500 transition-all mb-6">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="full_description" rows="5" placeholder="Detailed description for the landing page..." class="w-full bg-white/5 border border-white/10 rounded-[2rem] px-8 py-6 text-sm font-medium text-slate-300 focus:outline-none focus:border-emerald-500 transition-all resize-none mb-6">{{ old('full_description') }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3" class="w-full bg-white/5 border border-white/10 rounded-[2rem] px-8 py-6 text-sm font-medium text-slate-300 focus:outline-none focus:border-emerald-500 transition-all resize-none">{{ old('description') }}</textarea>
                        </div>
