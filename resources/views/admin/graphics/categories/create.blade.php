    ...
    <div x-data="{ 
        ...
        <form action="{{ route('admin.graphics.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            ...
            <div x-show="hasDetails" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-10">
                ...
                <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10">
                    ...
                    <div class="space-y-8">
                        <div>
                            ...
                            <textarea name="description" rows="4" class="w-full bg-white/5 border border-white/10 rounded-[2rem] px-8 py-6 text-sm font-medium text-slate-300 focus:outline-none focus:border-emerald-500 transition-all resize-none">{{ old('description') }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div x-data="{ preview: null }">
                                <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Before Image (Sample)</label>
                            ...
                            <div x-data="{ preview: null }">
                                <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">After Image (Sample)</label>
                                <div class="flex items-center gap-6 p-4 bg-white/5 border border-white/10 rounded-2xl">
                                    <div class="w-24 h-24 rounded-xl bg-slate-800 overflow-hidden shrink-0 border border-white/5">
                                        <template x-if="preview">
                                            <img :src="preview" class="w-full h-full object-cover">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input type="number" step="0.01" name="starting_price" value="{{ old('starting_price') }}" placeholder="0.49" 
                            ...
                            <div>
                                ...
                                <input type="text" name="price_unit" value="{{ old('price_unit', 'per image') }}" placeholder="per image" 
                                    class="w-full bg-white/5 border border-white/10 rounded-xl px-6 py-3 text-sm font-bold text-white focus:outline-none focus:border-emerald-500 transition-all">
                            </div>
                        </div>
                    </div>
                ...
                {{-- Interactive Components (FAQs, Methods) --}}
