...
@section('title', 'Edit Category | Graphics Studio')
...
<div class="p-8">
    <div class="mb-10 reveal">
        ...
        <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">Edit Category</h1>
        <p class="text-slate-400 font-medium">Update the primary grouping: {{ $category->name }}</p>
    </div>
    ...
    <div class="glass-card rounded-[32px] border-white/5 shadow-2xl relative overflow-hidden reveal reveal-delay-1 p-10 max-w-2xl">
        <form action="{{ route('admin.graphics.categories.update', $category) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Category Name</label>
                <input type="text" name="name" value="{{ old('name', $category->name) }}" placeholder="e.g. Image Editing" required 
                ...
                <p class="mt-2 text-[10px] text-slate-500 font-medium ml-1 italic">Changing the name will also update the slug used in URLs.</p>
            </div>
