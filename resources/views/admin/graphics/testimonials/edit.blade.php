...
@section('title', 'Refine VOX | Graphics Studio')
...
<div class="p-4 md:p-8 max-w-5xl mx-auto overflow-x-hidden">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 reveal">
        <div>
            <h1 class="text-3xl md:text-4xl font-black text-white mb-2 tracking-tight">Refine VOX</h1>
            <p class="text-slate-400 font-medium">Updating the endorsement of: <span class="text-indigo-400">"{{ $testimonial->name }}"</span></p>
        </div>
    ...
    <form action="{{ route('admin.graphics.testimonials.update', $testimonial->id) }}" method="POST" enctype="multipart/form-data" class="grid lg:grid-cols-12 gap-8">
        @csrf
        @method('PUT')
        ...
        <div class="lg:col-span-4 lg:sticky lg:top-8">
            <div class="glass-card p-8 rounded-[2.5rem] border-white/5 shadow-2xl flex flex-col items-center text-center">
                <label class="block text-[10px] uppercase tracking-[0.2em] font-black text-slate-500 mb-6 w-full">Current Avatar</label>
                <div onclick="document.getElementById('avatar').click()" class="image-upload-wrapper group">
                    @if($testimonial->avatar)
                        <img id="preview_img" class="avatar-preview" src="{{ asset('storage/' . $testimonial->avatar) }}" alt="Preview">
                    @else
                        <img id="preview_img" class="avatar-preview hidden" src="#" alt="Preview">
                    @endif
                    <div id="upload_placeholder" class="{{ $testimonial->avatar ? 'hidden' : '' }} text-center p-4 transition-transform duration-500 group-hover:scale-110">
                        <i class="ri-user-add-line text-3xl text-slate-600 mb-2 block"></i>
                        <span class="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Update Photo</span>
                    </div>
                </div>
                <input type="file" id="avatar" name="avatar" accept="image/*" class="hidden" onchange="previewImage(this)">
                <p class="text-[9px] text-slate-500 mt-6 font-medium italic">Changes will reflect immediately in the public VOX section.</p>
            </div>
        </div>
            ...
            <div class="glass-card p-8 rounded-[2.5rem] border-white/5 shadow-2xl space-y-8">
                ...
                <div>
                    ...
                    <textarea name="content" rows="6" required placeholder="Paste the client testimonial here..." 
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-medium text-slate-300 focus:outline-none focus:border-indigo-500 transition-all resize-none">{{ old('content', $testimonial->content) }}</textarea>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-6 items-end">
                    <div>
                        ...
                        <select name="rating" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all select-dark">
                            <option value="5" {{ $testimonial->rating == 5 ? 'selected' : '' }}>5 - Exceptional</option>
                            <option value="4" {{ $testimonial->rating == 4 ? 'selected' : '' }}>4 - High Quality</option>
                            <option value="3" {{ $testimonial->rating == 3 ? 'selected' : '' }}>3 - Standard</option>
                            <option value="2" {{ $testimonial->rating == 2 ? 'selected' : '' }}>2 - Below Expectation</option>
                    ...
                    <div>
                        ...
                        <select name="category_id" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all select-dark">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ $testimonial->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="service_id" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all select-dark">
                            ...
                            @foreach($services as $svc)
                                <option value="{{ $svc->id }}" {{ $testimonial->service_id == $svc->id ? 'selected' : '' }}>{{ $svc->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between p-4 bg-slate-900/50 rounded-2xl border border-white/5 mt-6">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" class="sr-only peer" {{ $testimonial->is_active ? 'checked' : '' }}>
        ...
        function previewImage(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    $('#preview_img').removeClass('hidden').fadeIn(600);
