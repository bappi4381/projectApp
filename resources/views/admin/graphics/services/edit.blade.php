        ...
        <form action="{{ route('admin.graphics.services.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            ...
            <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10 reveal reveal-delay-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div>
                        <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Parent Service (Conditional)</label>
                        <select name="parent_id" class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all select-dark">
                            <option value="">-- No Parent (Primary Service) --</option>
                            @foreach($parentServices as $ps)
                                @if($ps->id != $service->id) {{-- Prevent self-reference --}}
                                    <option value="{{ $ps->id }}" {{ old('parent_id', $service->parent_id) == $ps->id ? 'selected' : '' }}>{{ $ps->name }}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $service->name) }}" required class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-indigo-500 transition-all">
                    </div>
                </div>
