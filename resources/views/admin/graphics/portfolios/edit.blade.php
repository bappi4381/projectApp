...
@section('title', 'Edit Portfolio Item | Graphics Studio')
...
<div class="p-8">
    <div class="mb-10 reveal">
        ...
        <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">Edit Portfolio Item</h1>
        <p class="text-slate-400 font-medium text-sm">Update the details of your showcase piece</p>
    </div>

    <form action="{{ route('admin.graphics.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl space-y-8 reveal reveal-delay-1">
        @csrf
        @method('PUT')
        ...
        <div class="glass-card rounded-[24px] border-white/5 p-8 shadow-2xl space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $portfolio->title) }}" required class="w-full bg-slate-800/50 border border-white/5 rounded-xl px-5 py-3 text-white focus:outline-none focus:border-indigo-500/50 transition-all">
                </div>
                <div class="space-y-2">
                    <label class="block text-[11px] font-black text-slate-400 uppercase tracking-widest pl-1">Category</label>
                    <input type="text" name="category" value="{{ old('category', $portfolio->category) }}" required class="w-full bg-slate-800/50 border border-white/5 rounded-xl px-5 py-3 text-white focus:outline-none focus:border-indigo-500/50 transition-all">
                </div>
            </div>

            <div class="space-y-2">
                <label class="block text-[11px] font-black text-slate-400 uppercase tracking-widest pl-1">Description</label>
                <textarea name="description" rows="4" class="w-full bg-slate-800/50 border border-white/5 rounded-xl px-5 py-3 text-white focus:outline-none focus:border-indigo-500/50 transition-all">{{ old('description', $portfolio->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-4">
                    ...
                    <div class="relative group aspect-square rounded-2xl border-2 border-dashed border-white/10 overflow-hidden flex flex-col items-center justify-center bg-slate-800/30 hover:bg-slate-800/50 transition-all">
                        ...
                        <img id="before_preview" src="{{ $portfolio->before_image ? asset('storage/' . $portfolio->before_image) : '' }}" class="absolute inset-0 w-full h-full object-cover {{ $portfolio->before_image ? '' : 'hidden' }}">
                        <div class="text-center group-hover:scale-110 transition-transform p-4">
                            ...
                            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Change Original</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative group aspect-square rounded-2xl border-2 border-dashed border-white/10 overflow-hidden flex flex-col items-center justify-center bg-slate-800/30 hover:bg-slate-800/50 transition-all">
                        ...
                        <img id="after_preview" src="{{ $portfolio->after_image ? asset('storage/' . $portfolio->after_image) : '' }}" class="absolute inset-0 w-full h-full object-cover {{ $portfolio->after_image ? '' : 'hidden' }}">
                        <div class="text-center group-hover:scale-110 transition-transform p-4">
                            <i class="ri-upload-cloud-2-line text-4xl text-slate-600 block mb-2"></i>
                            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Change Edited</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="number" name="order" value="{{ $portfolio->order }}" class="w-full bg-slate-800/50 border border-white/5 rounded-xl px-5 py-3 text-white focus:outline-none focus:border-indigo-500/50 transition-all">
                </div>
                <div class="space-y-2">
                    ...
                    <select name="is_active" class="w-full bg-slate-800/50 border border-white/5 rounded-xl px-5 py-3 text-white focus:outline-none focus:border-indigo-500/50 transition-all appearance-none cursor-pointer">
                        <option value="1" {{ $portfolio->is_active ? 'selected' : '' }}>Active (Published)</option>
                        <option value="0" {{ !$portfolio->is_active ? 'selected' : '' }}>Draft (Hidden)</option>
                    </select>
                </div>
                <div class="space-y-2">
                    ...
                    <select name="show_on_home" class="w-full bg-slate-800/50 border border-white/5 rounded-xl px-5 py-3 text-white focus:outline-none focus:border-indigo-500/50 transition-all appearance-none cursor-pointer">
                        <option value="0" {{ !$portfolio->show_on_home ? 'selected' : '' }}>No</option>
                        <option value="1" {{ $portfolio->show_on_home ? 'selected' : '' }}>Yes (Show in Slider)</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end pr-4">
            <button type="submit" class="px-10 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl transition-all shadow-xl shadow-indigo-500/20 flex items-center gap-3 active:scale-95">
                <i class="ri-save-line text-xl"></i> Update Portfolio Item
            </button>
        </div>
    </form>
</div>
