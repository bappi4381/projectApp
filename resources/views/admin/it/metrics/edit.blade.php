...
@section('title', 'Edit Success Metric | IT Command Center')
...
<div class="p-8 max-w-[1000px] mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 reveal">
        <div>
            <div class="flex items-center gap-3 mb-2">
                ...
                <span class="text-[10px] font-black text-cyan-500 uppercase tracking-[0.3em]">Modify Configuration</span>
            </div>
            <h1 class="text-4xl font-black text-white tracking-tight">Edit Metric</h1>
        </div>
    ...
    <div class="glass-card rounded-[32px] border-white/5 p-10 reveal">
        <form action="{{ route('admin.it.metrics.update', $metric->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-3">
                    ...
                    <input type="text" name="title" value="{{ old('title', $metric->title) }}" required placeholder="e.g. Projects Completed"
                ...
                <div class="space-y-3">
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $metric->icon) }}" required placeholder="ri-macbook-line"
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all">
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-3">
                    ...
                    <input type="text" name="count" value="{{ old('count', $metric->count) }}" required placeholder="e.g. 7000"
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all">
                </div>
                <div class="space-y-3">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Value Suffix (Optional)</label>
                    <input type="text" name="suffix" value="{{ old('suffix', $metric->suffix) }}" placeholder="e.g. +"
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all">
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-8 mb-10">
                <div class="space-y-3">
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $metric->sort_order) }}" required
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all">
                </div>
                <div class="flex items-center gap-4 pt-10">
                    <label class="relative inline-flex items-center cursor-pointer group">
                        <input type="checkbox" name="is_active" value="1" {{ $metric->is_active ? 'checked' : '' }} class="sr-only peer">
