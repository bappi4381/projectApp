...
@section('title', 'Edit Software | Admin')
...
<div class="p-8">
    ...
    <div class="mb-10 reveal">
        ...
        <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">Edit Software</h1>
        <p class="text-slate-400 font-medium text-sm italic">Modifying: <span class="text-cyan-400">{{ $software->name }}</span></p>
    </div>
    ...
    <form action="{{ route('admin.it.software.update', $software->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8"
        ...
        <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10 reveal reveal-delay-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $software->name) }}" required
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition-all shadow-inner">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                <div>
                    <label class="block text-[11px] uppercase tracking-widest font-black text-slate-500 mb-3 ml-1">Software Logo/Image</label>
                    <div class="relative w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus-within:border-cyan-500 focus-within:ring-2 focus-within:ring-cyan-500/20 transition-all shadow-inner flex items-center gap-4">
                        ...
                        <input type="file" name="image" accept="image/*" class="w-full bg-transparent text-slate-400 focus:outline-none cursor-pointer">
                    </div>
                    @if($software->image_url)
                        <div class="mt-4 flex items-center gap-4">
                            <img src="{{ Str::startsWith($software->image_url, 'http') ? $software->image_url : asset('storage/' . $software->image_url) }}" alt="Current Image" class="w-16 h-16 rounded-2xl object-cover border border-white/10">
                            <p class="text-[10px] text-slate-500 font-black uppercase tracking-widest">Current Image</p>
                        </div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="link_url" value="{{ old('link_url', $software->link_url) }}"
        ...
        <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10 reveal reveal-delay-2">
            ...
            <div class="space-y-8">
                <div>
                    ...
                    <input type="text" name="short_desc" value="{{ old('short_desc', $software->short_desc) }}" required
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-bold text-white focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition-all shadow-inner">
                </div>
                ...
                <div>
                    ...
                    <textarea name="long_desc" rows="6" required
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-sm font-medium text-slate-300 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition-all resize-none shadow-inner">{{ old('long_desc', $software->long_desc) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10 reveal reveal-delay-2" x-data="{
            solutions: {{ json_encode(old('solutions', !empty($software->solutions) ? $software->solutions : [['title' => '', 'logoText' => '', 'header' => '', 'desc' => '', 'btnText' => '', 'quote' => '', 'author' => '', 'image' => '', 'link' => '']])) }},
            addSolution() {
                    ...
                    <div class="p-6 bg-slate-900/50 rounded-2xl border border-white/5 relative group">
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="hidden" :name="`solutions[${index}][image]`" x-model="sol.image">
                                <template x-if="sol.image">
                                    <div class="mt-3 flex items-center gap-3">
                                        <img :src="sol.image && sol.image.startsWith('http') ? sol.image : '{{ asset('storage') }}/' + sol.image" class="w-12 h-12 rounded-lg object-cover border border-white/10">
                                        <span class="text-[9px] text-slate-500 font-black uppercase tracking-widest">Current Mockup</span>
                                    </div>
                                </template>
                            </div>
                        </div>
        ...
        <div class="glass-card rounded-[32px] border-white/5 shadow-2xl p-10 reveal reveal-delay-3">
            ...
            <div class="p-6 bg-emerald-500/5 rounded-3xl border border-emerald-500/10 flex items-center justify-between shadow-sm max-w-sm">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" class="sr-only peer" value="1" {{ $software->is_active ? 'checked' : '' }}>
        ...
        <div class="flex justify-end gap-4 pt-2">
            ...
            <button type="submit" :disabled="isSubmitting"
                ...
                <template x-if="!isSubmitting"><span>Update Software</span></template>
