...
<div class="p-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 reveal">
        <div>
            <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">IT Services</h1>
            <p class="text-slate-400 font-medium text-sm">Manage enterprise IT solutions and technical services.</p>
        </div>
        <a href="{{ route('admin.it.services.create') }}" class="group relative px-8 py-4 bg-cyan-600 rounded-2xl transition-all hover:scale-105 active:scale-95 shadow-xl shadow-cyan-500/20 overflow-hidden flex items-center gap-3 self-start">
            ...
            <span class="relative text-white font-black uppercase tracking-widest text-[11px]">Add IT Service</span>
        </a>
    </div>
    ...
    <div class="glass-card rounded-[24px] border-white/5 p-6 mb-6 flex flex-wrap gap-4 items-center reveal reveal-delay-1">
        <form action="{{ route('admin.it.services.index') }}" method="GET" class="flex flex-wrap gap-4 flex-1">
            ...
            @if(request()->anyFilled(['search']))
                <a href="{{ route('admin.it.services.index') }}" class="px-6 py-3 bg-white/5 text-slate-400 hover:text-white text-[11px] font-black uppercase rounded-xl transition-all">Reset</a>
            @endif
        </form>
    </div>
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="hover:bg-white/[0.02] transition-all group">
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-cyan-500/10 border-cyan-500/20 text-cyan-400 flex items-center justify-center border group-hover:scale-110 group-hover:border-cyan-500/40 transition-all shadow-lg">
                                    <i class="{{ $service->icon ?? 'ri-server-line' }} text-lg"></i>
                                </div>
                                <div>
                                    <h4 class="font-black text-white text-[13px] tracking-tight group-hover:text-cyan-400 transition-colors">{{ $service->name }}</h4>
                        ...
                        <td class="px-8 py-5">
                            ...
                            @else
                                <span class="text-slate-700 text-[10px] font-black uppercase tracking-widest italic">— Contact for Quote —</span>
                            @endif
                        </td>

                        <td class="px-8 py-5">
                            @if($service->has_details)
                                <span class="flex items-center gap-1.5 text-[9px] font-black text-cyan-400 uppercase tracking-[0.15em] bg-cyan-500/10 px-2 py-0.5 rounded-full w-fit border border-cyan-500/20">
                                    <i class="ri-pages-line"></i> Detailed View
                                </span>
                            @else
                        ...
                        <td class="px-8 py-5 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.it.services.edit', $service) }}"
                                ...
                                <form action="{{ route('admin.it.services.destroy', $service) }}" method="POST"
                                    onsubmit="return confirm('Delete this IT service?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center text-slate-500 font-medium">
                            ...
                            No IT services found. <a href="{{ route('admin.it.services.create') }}" class="text-cyan-400 font-bold hover:underline">Create the first one.</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
