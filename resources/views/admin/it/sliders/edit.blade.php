...
@section('title', 'Edit Hero Slider | IT Command Center')
...
<div class="p-8 max-w-[1000px] mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 reveal">
        <div>
            <div class="flex items-center gap-3 mb-2">
                ...
                <span class="text-[10px] font-black text-cyan-500 uppercase tracking-[0.3em]">Modify Deployment</span>
            </div>
            <h1 class="text-4xl font-black text-white tracking-tight">Edit Hero Slider</h1>
        </div>
    ...
    <div class="glass-card rounded-[32px] border-white/5 p-10 reveal">
        <form action="{{ route('admin.it.sliders.update', $slider->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-3 mb-8">
                ...
                <textarea name="title" rows="2" required placeholder="e.g. Customize Software Development"
                    class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-lg focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all resize-none">{{ old('title', $slider->title) }}</textarea>
            </div>

            <div class="grid md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-3">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Sub-heading</label>
                    <input type="text" name="subtitle" value="{{ old('subtitle', $slider->subtitle) }}" placeholder="e.g. Innovating the Future"
                ...
                <div class="space-y-3">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Update Background Image</label>
                    ...
                    @if($slider->image)
                        <p class="text-[9px] text-cyan-500 font-bold mt-2 uppercase tracking-widest">Current: {{ basename($slider->image) }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-3">
                    ...
                    <input type="text" name="btn_text" value="{{ old('btn_text', $slider->btn_text) }}" placeholder="e.g. LEARN MORE"
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all">
                </div>
                <div class="space-y-3">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">Button URL</label>
                    <input type="text" name="btn_url" value="{{ old('btn_url', $slider->btn_url) }}" placeholder="e.g. /services"
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all">
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-8 mb-10">
                <div class="space-y-3">
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $slider->sort_order) }}" required
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white font-bold text-sm focus:outline-none focus:border-cyan-500 focus:bg-white/10 transition-all">
                </div>
