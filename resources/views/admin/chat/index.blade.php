...
<div class="p-8 h-[calc(100vh-80px)] overflow-hidden flex flex-col">
    ...
    <div class="flex-grow overflow-hidden flex gap-6">
                    ...
                    @foreach($users as $user)
                        <button onclick="loadMessages({{ $user->id }})" 
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-bold shrink-0">
                                ...
                                <span class="absolute right-4 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_#ef4444] d-none" id="unread-{{ $user->id }}"></span>
                            </div>
                        </button>
                    @endforeach
                </div>
        ...
        <div class="flex-grow flex flex-col relative">
            <!-- Chat Window -->
            <div class="glass-card rounded-2xl flex flex-col h-full border-white/5 overflow-hidden" id="chatWindow" style="display: none;">
                <!-- Header -->
                <div class="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.02] shrink-0">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold" id="activeUserAvatar">?</div>
                        <div>
                            ...
                            <div class="flex items-center gap-1.5 mt-0.5">
                                ...
                                <span class="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">Active Now</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Messages -->
                <div class="flex-grow overflow-y-auto p-6 space-y-4 custom-sidebar-scroll bg-slate-900/40" id="messagesContainer">
                ...
                <div class="p-4 bg-white/[0.02] border-t border-white/5 shrink-0">
                    <form id="adminChatForm" class="flex gap-3">
                        ...
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white w-12 h-12 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-600/20 transition-all">
            ...
            <!-- Empty State -->
            <div class="glass-card rounded-2xl flex flex-col items-center justify-center text-center h-full border-white/5" id="chatEmptyState">
                <div class="w-20 h-20 rounded-3xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 mb-6">
                    <i class="ri-message-3-line text-4xl"></i>
                </div>
                <h3 class="text-xl font-bold text-white mb-2">No Conversation Selected</h3>
                <p class="text-slate-400 max-w-xs mx-auto">Select a guest from the left sidebar to view message history and respond.</p>
            </div>
        </div>
    </div>
</div>
...
.custom-sidebar-scroll::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.05); border-radius: 20px; }
.message-bubble { max-width: 80%; padding: 12px 16px; border-radius: 20px; font-size: 13.5px; line-height: 1.5; }
.message-guest { background: rgba(255,255,255,0.05); color: #cbd5e1; border: 1px solid rgba(255,255,255,0.05); border-bottom-left-radius: 4px; }
.message-admin { background: #4f46e5; color: white; border-bottom-right-radius: 4px; box-shadow: 0 4px 15px rgba(79, 70, 229, 0.2); }
</style>
