                ...
                @else
                    @foreach($videoSubCategories as $subCategory)
                        <div x-data="{ isOpen: false }" @mouseenter="isOpen = true" @mouseleave="isOpen = false" class="relative group">
                            ...
                            {{-- Dropdown --}}
                            @if($subCategory->services->count())
                            <div x-show="isOpen" x-cloak class="absolute top-full left-0 pt-6 z-[60]"
                                ...
                                <div class="bg-white rounded shadow-2xl border border-slate-200 py-6 px-8 min-w-[240px]">
                                    <ul class="space-y-3">
                                        @foreach($subCategory->services as $svc)
                                            <li>
                                                @if($svc->has_details)
                                                    <a href="{{ route('graphics.service-detail', $svc->slug) }}" class="text-[12px] font-bold text-slate-500 hover:text-indigo-600 flex items-center gap-3 transition-colors">
